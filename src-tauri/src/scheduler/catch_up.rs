...
impl SchedulerState {
    ...
    pub fn has_cycle(&self, cycle_id: &str) -> bool { self.completed_cycles.contains(cycle_id) }
    pub fn cycles(&self) -> impl Iterator<Item = &String> { self.completed_cycles.iter() }
}

pub struct CatchUp;

impl CatchUp {
    pub fn should_run(cycle_id: &str, now: i64, state: &SchedulerState, schedule: &Schedule) -> bool {
        !state.has_cycle(cycle_id)
            && schedule.due_for_cycle(cycle_id).is_some_and(|due| now >= due)
    }
}
