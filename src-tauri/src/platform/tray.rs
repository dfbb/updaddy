...
pub fn setup(app: &AppHandle) -> Result<(), String> {
    ...
    let mut tray = TrayIconBuilder::with_id("updaddy").menu(&menu).tooltip("updaddy");
    if let Some(icon) = app.default_window_icon() { tray = tray.icon(icon.clone()).icon_as_template(true); }
    tray.on_menu_event(|app, event| match event.id().as_ref() {
        "open" | "count" => { if let Some(w)=app.get_webview_window("main") { let _=w.show(); let _=w.set_focus(); } },
        "settings" => { let _ = crate::commands::open_settings(app.clone()); },
        "update" => { let _=app.emit("tray-update-all", ()); }, _=>{}
...
fn build_menu(app: &AppHandle) -> Result<tauri::menu::Menu<tauri::Wry>, String> {
    ...
    let update = MenuItem::with_id(app, "update", "一键更新", app.state::<crate::commands::AppState>().supervisor.active_task_count()==0, None::<&str>).map_err(|e| e.to_string())?;
    ...
    MenuBuilder::new(app).items(&[&count, &update, &open, &settings, &quit]).build().map_err(|e| e.to_string())
}

pub fn refresh(app: &AppHandle) {
    if let Some(tray) = app.tray_by_id("updaddy") {
        if let Ok(menu) = build_menu(app) { let _ = tray.set_menu(Some(menu)); }
    }
}
