...
export interface PackageTask { task_id: string; ecosystem: Ecosystem; name: string; operation: string; status: TaskStatus; error?: string; completed?: number; total?: number; eta_seconds?: number; phase?: string; message?: string; }
export interface OperationBatch { batch_id: string; ecosystem: Ecosystem; tasks: PackageTask[]; created_at: number; }
export interface LogEntry { message: string; emitted_at: number; stream: string; task_id?: string; }
